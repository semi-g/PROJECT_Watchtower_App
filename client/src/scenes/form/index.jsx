...
//const phoneRegEx = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{2,3}[-\s\.]?[0-9]{5}$/im
const phoneRegEx = /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/im
const userSchema = yup.object().shape({
    //Use of pre-made validation functions
                    ...
                    <form onSubmit={ handleSubmit }>
                        ...
                        >
                            <TextField fullWidth variant="filled" type="text" label="First Name" 
                                ...
                                {/* Double negative + touched here means: 
                                        only when the entry field is 'touched' (that is: when the field is entered and exited)
                                        and it's not filled in (so both touched and not filled in) -> show 'required' error message. 
                                        But if one of those becomes not true, the message is not shown.*/}
                            ...
                            <TextField fullWidth variant="filled" type="text" label="Contact" 
